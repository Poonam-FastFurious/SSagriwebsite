function Whychooseus() {
  return (
    <>
      <section className="feature-area whight-feature grey-bg feature-top ">
        <div className="container">
          <div className="feature-bg-round  ">
            <div className="tpfeature-border">
              <div className="row row-cols-lg-5 row-cols-md-3 row-cols-1">
                <div className="col">
                  <div className="mainfeature__item   mb-45">
                    <div className="mainfeature__icon">
                      <img
                        src="https://html.hixstudio.net/orfarm/assets/img/icon/feature-icon-6.svg"
                        alt=""
                      />
                    </div>
                    <div className="mainfeature__content">
                      <h4 className="mainfeature__title">Fast Delivery</h4>
                      <p>Across India</p>
                    </div>
                  </div>
                </div>
                <div className="col">
                  <div className="mainfeature__item  mb-45">
                    <div
                      className="mainfeature__icon"
                      style={{ color: "white" }}
                    >
                      <img
                        src="https://html.hixstudio.net/orfarm/assets/img/icon/feature-icon-7.svg"
                        alt=""
                      />
                    </div>
                    <div className="mainfeature__content">
                      <h4 className="mainfeature__title">safe payment</h4>
                      <p>100% Secure Payment</p>
                    </div>
                  </div>
                </div>
                <div className="col">
                  <div className="mainfeature__item  mb-45">
                    <div className="mainfeature__icon">
                      <img
                        src="https://html.hixstudio.net/orfarm/assets/img/icon/feature-icon-8.svg"
                        alt=""
                      />
                    </div>
                    <div className="mainfeature__content">
                      <h4 className="mainfeature__title">Online Discount</h4>
                      <p>Add Multi-buy Discount </p>
                    </div>
                  </div>
                </div>
                <div className="col">
                  <div className="mainfeature__item  mb-45">
                    <div className="mainfeature__icon">
                      <img
                        src="https://html.hixstudio.net/orfarm/assets/img/icon/feature-icon-9.svg"
                        alt=""
                      />
                    </div>
                    <div className="mainfeature__content">
                      <h4 className="mainfeature__title">Help Center</h4>
                      <p> Dedicated 24/7 Support </p>
                    </div>
                  </div>
                </div>
                <div className="col">
                  <div className="mainfeature__item  mb-45">
                    <div className="mainfeature__icon">
                      <img
                        src="https://html.hixstudio.net/orfarm/assets/img/icon/feature-icon-10.svg"
                        alt=""
                      />
                    </div>
                    <div className="mainfeature__content">
                      <h4 className="mainfeature__title">Curated items</h4>
                      <p>From Handpicked Sellers</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}

export default Whychooseus;
